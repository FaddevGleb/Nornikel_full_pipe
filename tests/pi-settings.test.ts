...
import { normalizeFeynmanSettings, normalizeThinkingLevel, parseModelSpec } from "../src/pi/settings.js";
...
test("parseModelSpec splits on '/' even when the model id embeds a colon (Yandex gpt:// URIs)", () => {
	const { registry, calls } = fakeModelRegistry();
	const result = parseModelSpec("yandex-ai-studio/gpt://b1gexamplefolderid/yandexgpt/latest", registry);
	assert.deepEqual(calls, [{ provider: "yandex-ai-studio", id: "gpt://b1gexamplefolderid/yandexgpt/latest" }]);
	assert.deepEqual(result, { provider: "yandex-ai-studio", id: "gpt://b1gexamplefolderid/yandexgpt/latest" });
});

test("parseModelSpec still supports the provider:model shorthand", () => {
	const { registry, calls } = fakeModelRegistry();
	parseModelSpec("anthropic:claude-sonnet-4", registry);
	assert.deepEqual(calls, [{ provider: "anthropic", id: "claude-sonnet-4" }]);
});

test("parseModelSpec supports plain provider/model specs", () => {
	const { registry, calls } = fakeModelRegistry();
	parseModelSpec("openrouter/openai/gpt-5.5", registry);
	assert.deepEqual(calls, [{ provider: "openrouter", id: "openai/gpt-5.5" }]);
});
