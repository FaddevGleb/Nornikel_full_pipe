...
export function parseModelSpec(spec: string, modelRegistry: ModelRegistry) {
	...
	// Prefer "/" as the provider/model separator whenever it appears before any
	// ":" (matches resolveAvailableModelSpec's convention in model/commands.ts).
	// This matters for model ids that embed a colon themselves, e.g. Yandex AI
	// Studio's "gpt://<folder-id>/<model>/<version>" URI: naively picking ":"
	// whenever the spec contains one would split "yandex-ai-studio/gpt://..."
	// into the bogus provider "yandex-ai-studio/gpt".
