...
test("upsertProviderConfig writes Yandex AI Studio config with folder-id header", () => {
	const dir = mkdtempSync(join(tmpdir(), "feynman-yandex-"));
	const modelsPath = join(dir, "models.json");

	const result = upsertProviderConfig(modelsPath, "yandex-ai-studio", {
		baseUrl: "https://llm.api.cloud.yandex.net/v1",
		apiKey: "YANDEX_API_KEY",
		api: "openai-completions",
		authHeader: true,
		headers: { "x-folder-id": "b1gexamplefolderid", "x-data-logging-enabled": "false" },
		models: [{ id: "gpt://b1gexamplefolderid/yandexgpt/latest" }],
	});
	assert.deepEqual(result, { ok: true });

	const parsed = JSON.parse(readFileSync(modelsPath, "utf8")) as any;
	assert.equal(parsed.providers["yandex-ai-studio"].baseUrl, "https://llm.api.cloud.yandex.net/v1");
	assert.equal(parsed.providers["yandex-ai-studio"].apiKey, "YANDEX_API_KEY");
	assert.equal(parsed.providers["yandex-ai-studio"].api, "openai-completions");
	assert.equal(parsed.providers["yandex-ai-studio"].authHeader, true);
	assert.deepEqual(parsed.providers["yandex-ai-studio"].headers, {
		"x-folder-id": "b1gexamplefolderid",
		"x-data-logging-enabled": "false",
	});
	assert.deepEqual(parsed.providers["yandex-ai-studio"].models, [{ id: "gpt://b1gexamplefolderid/yandexgpt/latest" }]);
});
