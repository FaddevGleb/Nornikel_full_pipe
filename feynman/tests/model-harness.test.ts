...
import { buildYandexModelUri, isLocalModelProvider, resolveModelProviderForCommand, setDefaultModelSpec } from "../src/model/commands.js";
...
test("chooseRecommendedModel selects Yandex AI Studio when it is the only configured provider", () => {
	withoutModelEnv(() => {
		const authPath = createAuthPath({});
			...
			JSON.stringify({
				providers: {
					"yandex-ai-studio": {
						...
						authHeader: true,
						headers: { "x-folder-id": "b1gexamplefolderid" },
						models: [{ id: buildYandexModelUri("b1gexamplefolderid", "yandexgpt", "latest") }],
					},
				},
			}) + "\n",
			"utf8",
		...
		assert.equal(recommendation?.spec, "yandex-ai-studio/gpt://b1gexamplefolderid/yandexgpt/latest");
	});
});
...
test("resolveModelProviderForCommand supports Yandex AI Studio as a first-class API-key provider", () => {
	const authPath = createAuthPath({});

	const resolved = resolveModelProviderForCommand(authPath, "yandex-ai-studio");

	assert.equal(resolved?.kind, "api-key");
	assert.equal(resolved?.id, "yandex-ai-studio");
});

test("buildYandexModelUri formats the gpt:// model URI Yandex AI Studio expects", () => {
	assert.equal(
		buildYandexModelUri("b1gexamplefolderid", "yandexgpt", "latest"),
		"gpt://b1gexamplefolderid/yandexgpt/latest",
	);
	assert.equal(
		buildYandexModelUri("b1gexamplefolderid", "yandexgpt-lite", "rc"),
		"gpt://b1gexamplefolderid/yandexgpt-lite/rc",
	);
});

test("isLocalModelProvider treats Yandex AI Studio as a remote (non-local) provider", () => {
	const authPath = createAuthPath({});

	assert.equal(isLocalModelProvider(authPath, "yandex-ai-studio"), false);
});
