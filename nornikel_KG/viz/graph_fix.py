...
и обновляет текстовые поля онтологических узлов из ConceptDictionary.
"""
import argparse
import json
import logging
...
def save_graph(graph_data, output_file, logger):
    try:
        ...
        logger.warning(f"Schema validation warning after modifications: {e}")
        with open(output_file, "w", encoding="utf-8") as f:
            json.dump(graph_data, f, ensure_ascii=False, indent=2)
        logger.info(f"Graph saved to: {output_file} (despite schema warnings)")
    except Exception as e:
