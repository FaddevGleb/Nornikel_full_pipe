...
function shouldSkipIncomingName(name) {
...
async function listFiles(dir) {
  try {
    ...
    return entries
      .filter((entry) => entry.isFile() && !shouldSkipIncomingName(entry.name))
...
export async function convertPendingIncoming(onLog) {
  ...
  for (const name of pending) {
    const inputPath = path.join(incomingDir, name);
    onLog?.({ level: 'info', message: `Converting ${name}` });
    await runDocConvertFile(inputPath, rawDir, onLog);
  }
...
const NOISY_STDERR_RE = /tesseract|img2table/i;

function runDocConvertFile(inputPath, rawDir, onLog) {
  return new Promise((resolve, reject) => {
    const python = configManager.getPythonExecutable();
    const args = [
      '-m', 'doc_converter.cli', 'convert', inputPath,
      '--format', 'md',
      '--output-dir', rawDir,
      '--vlm-backend', 'off',
    ...
    child.stderr.on('data', (chunk) => {
      for (const line of chunk.toString('utf8').split(/\r?\n/)) {
        ...
        if (!trimmed || NOISY_STDERR_RE.test(trimmed)) continue;
        onLog?.({ level: 'warn', message: trimmed });
      }
    });
    child.on('error', reject);
    child.on('close', (code) => {
      if (code === 0) resolve();
      else reject(new Error(`doc-convert failed for ${path.basename(inputPath)} (exit ${code})`));
    });
  });
}
