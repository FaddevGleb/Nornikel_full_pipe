...
import { convertPendingIncoming } from './documentConverterService.js';
...
export class PipelineRunner {
  ...
  async ensureDirectories() {
    const dirs = [
      'data/incoming', 'data/raw', 'data/raw/media',
      'data/staging', 'data/out', 'logs', 'viz/data/in', 'viz/data/out',
    ];
...
async runPipeline(stages, { incremental = false } = {}) {
  ...
  await convertPendingIncoming((entry) => {
    this.onLog({ level: entry.level ?? 'info', message: entry.message, stage: 'convert' });
  });
