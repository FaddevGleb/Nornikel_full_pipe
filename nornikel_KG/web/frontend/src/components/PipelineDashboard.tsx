...
import { api, type Job, subscribeJob } from '../api/client';
...
export function PipelineView() {
  ...
  useEffect(() => {
    ...
    api.listJobs().then(({ jobs }) => {
      ...
      const active = jobs.find((j) => ['running', 'interrupted', 'paused'].includes(j.status));

      if (active) attachJob(active.id);
      ...
      <div className="panel upload-panel">
        ...
        >
          ...
          <input
            ...
            accept=".txt,.md,.html,.pdf,.docx,.xlsx,.xls,.png,.jpg,.jpeg"
