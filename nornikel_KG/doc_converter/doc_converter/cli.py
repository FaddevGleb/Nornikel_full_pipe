...
from doc_converter.config import OutputFormat, PdfBackendChoice, Settings, VlmBackendChoice
...
class PdfBackendOption(str, Enum):
...
def _process_single_file(
    ...
    try:
        ...
        _write_outputs(stem, output_dir, settings.output_format, md_text, html_text)
...
def convert(
    ...
    vlm_backend: VlmBackendOption = typer.Option(
        VlmBackendOption.off,
        "--vlm-backend",
        help="Vision-language model backend",
    ),
    pdf_backend: PdfBackendOption = typer.Option(
        PdfBackendOption.pymupdf4llm,
        "--pdf-backend",
    ...
    settings = _build_settings(output_format, output_dir, vlm_backend, pdf_backend, ocr_lang)
    with Progress(
        SpinnerColumn(),
