...
| **pymupdf4llm** | PDF (быстро, без скачивания ML-моделей) | `python -c "import pymupdf4llm"` |
...
| `--pdf-backend` | `pymupdf4llm` | `pymupdf4llm` \| `pymupdf` |
| `--ocr-lang` | `ru` | Язык OCR для сканированных страниц PDF |
...
```powershell
# PDF — учебники и статьи (быстро, pymupdf4llm)
...
# Сканированный PDF: пустые страницы автоматически идут в PaddleOCR fallback
python -m doc_converter.cli convert "scan.pdf" --format md --output-dir ./out --ocr-lang ru
```

**Что происходит внутри:**

1. **pymupdf4llm** — markdown по страницам (текстовый слой, заголовки, таблицы)
2. Страницы без текста → **PaddleOCR** (кириллица)
3. **PyMuPDF** — embedded images → `out/media/`
4. Плохие таблицы → rerender страницы → img2table
...
settings = Settings(output_dir="out", VLM_BACKEND="off", PDF_BACKEND="pymupdf4llm")
...
# OCR для сканированных PDF-страниц
OCR_LANG=ru

# PDF backend: pymupdf4llm | pymupdf
...
| `PDF_BACKEND` | `pymupdf4llm` | Движок PDF: `pymupdf4llm` или `pymupdf` |
| `OCR_LANG` | `ru` | Язык PaddleOCR для сканов |
...
| PDF | `pdf_converter` | pymupdf4llm + PyMuPDF figures + PaddleOCR fallback |
...
### PDF: медленно или пустой текст

Проверьте `--pdf-backend pymupdf4llm`. Для сканов установите `paddleocr` и `--ocr-lang ru`.

### PDF: `ImportError: pymupdf4llm`
