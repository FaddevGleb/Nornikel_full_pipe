...
class SliceProcessor:
    def __init__(self, config, incremental=False):
        ...
        self._create_lightweight_dictionary()
        self.graph_nodes = []
        self.graph_edges = []
        self.node_ids = {}
    ...
    def _create_lightweight_dictionary(self):
        lightweight_concepts = []
        for c in self.concept_dict.get("concepts", []):
            term_data = c.get("term", {})
            search_terms = [term_data.get("primary", "")] + term_data.get("aliases", [])
            search_terms = [t for t in search_terms if t]
            lightweight_concepts.append({
                "concept_id": c["concept_id"],
                "ontology_class": c.get("ontology_class", "Concept"),
                "terms": search_terms
            })
        self.lightweight_concept_dict = {"concepts": lightweight_concepts}
        self.logger.info(f"Created lightweight concept index: {len(lightweight_concepts)} concepts")
    ...
    def _load_existing_graph(self):
        ...
        try:
            ...
            self.logger.info(
                ...
                f"{len(self.graph_edges)} edges, {len(self.processed_slice_ids)} processed slices"
            )
            print(f"Incremental mode: loaded {len(self.graph_nodes)} nodes, {len(self.graph_edges)} edges")
            print(f"  Already processed: {len(self.processed_slice_ids)} slices from {len(self.processed_source_files)} file(s)")
    ...
    def _format_slice_input(self, slice_data):
        input_obj = {
            "ConceptDictionary": self.lightweight_concept_dict,
            "Slice": {
    ...
    def _process_chunk_nodes(self, new_nodes):
        """Обрабатывает узлы с поддержкой онтологических классов."""
        nodes_to_add = []
        concept_lookup = {c["concept_id"]: c for c in self.concept_dict["concepts"]}
        for node in new_nodes:
            # === ЗАЩИТА: пропускаем строки и мусор от LLM ===
            if not isinstance(node, dict):
                self.logger.warning(f"Skipping non-dict node: {node}")
                continue
            node_id = node.get("id", "")
            ...
            elif node_id in concept_lookup:
                if node_id in self.node_ids:
                    continue
    ...
    def _validate_edges(self, edges):
        ...
        for edge in edges:
            if not isinstance(edge, dict):
                self.logger.warning(f"Skipping non-dict edge: {edge}")
                continue
            ...
            if conf < 0.5:
                continue
            if source not in node_type_lookup or target not in node_type_lookup:
                continue
            s_type = node_type_lookup[source]
            t_type = node_type_lookup[target]
    ...
    def _save_checkpoint(self):
        ...
        graph_stats = {"total_nodes": len(self.graph_nodes), "total_edges": len(self.graph_edges)}
        metadata = {
            "_meta": {
                "itext2kg_graph": {
                    "last_checkpoint_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "mode": "incremental",
        ...
        try:
            ...
            self.logger.debug(f"Checkpoint saved: {len(self.graph_nodes)} nodes, {len(self.graph_edges)} edges")
        except Exception as e:
        ...
        for attempt in range(max_retries + 1):
            try:
                if attempt == 0:
                    ...
                    # Защита от AttributeError, если API вернул ошибку и usage = None
                    if usage is not None:
                        self.api_usage["total_input_tokens"] += getattr(usage, 'input_tokens', 0) or 0
                        self.api_usage["total_output_tokens"] += getattr(usage, 'output_tokens', 0) or 0
                        self.stats.total_tokens_used += getattr(usage, 'total_tokens', 0) or 0
    ...
    def run(self):
        ...
        if self.incremental and self.processed_slice_ids:
            ...
            slice_files = [sf for sf in slice_files if self._get_slice_id(sf) not in self.processed_slice_ids]
        ...
        print(f"[{timestamp}] START    | {len(slice_files)} new slices | mode={mode} | model={model} | tpm={tpm_limit // 1000}k")
    ...
    def _enrich_nodes_from_dictionary(self):
        """
        Обогащает узлы графа полными метаданными из ConceptDictionary.
        Вызывается ПОСЛЕ построения графа, перед сохранением.
        
        Во время работы itext2kg_graph.py для облегчения промпта передавался
        только легкий словарь (concept_id, terms). Теперь подтягиваем обратно:
        - definition (полное определение концепта)
        - aliases (синонимы)
        - ontology_class (если отсутствовал)
        - любые другие поля из ConceptDictionary
        """
        # Строим lookup для быстрого доступа к полным данным концептов
        concept_lookup = {c["concept_id"]: c for c in self.concept_dict.get("concepts", [])}
        ...
        for node in self.graph_nodes:
            ...
            # Chunk и Assessment не обогащаем - у них свои данные
            if node_type in ["Chunk", "Assessment"]:
                continue
            
            # Ищем концепт в словаре
            concept_data = concept_lookup.get(node_id)
            if not concept_data:
                # Узла нет в словаре (например, галлюцинация LLM) - пропускаем
                skipped_count += 1
                continue
            
            # Обогащаем definition
            term_data = concept_data.get("term", {})
            definition = concept_data.get("definition", "")
            if definition and not node.get("definition"):
                node["definition"] = definition
            
            # Обогащаем text (используем primary + aliases в формате "Primary (alias1, alias2)")
            primary = term_data.get("primary", "")
            aliases = term_data.get("aliases", [])
            if primary:
                if aliases:
                    node["text"] = f"{primary} ({', '.join(aliases)})"
                else:
                    node["text"] = primary
            
            # Обогащаем name (короткое имя - только primary)
            if primary:
                node["name"] = primary
            
            # Обогащаем ontology_class (если отсутствует)
            if not node.get("ontology_class"):
                ontology_class = concept_data.get("ontology_class", "Concept")
                node["ontology_class"] = ontology_class
            
            # Добавляем aliases как отдельное поле для удобства downstream
            if aliases:
        ...
        self.logger.info(
            f"Enriched {enriched_count} nodes with metadata from ConceptDictionary "
            f"({skipped_count} skipped - not in dictionary)"
        )
        print(
            f"  Enriched {enriched_count} nodes with full metadata from ConceptDictionary"
        )
    ...
    def _finalize_and_save(self):
        output_path = OUTPUT_DIR / "LearningChunkGraph_raw.json"
        try:
            # === НОВОЕ: Обогащаем узлы полными метаданными перед сохранением ===
            self._enrich_nodes_from_dictionary()
...
def main():
    ...
    parser.add_argument("--incremental", action="store_true", help="Load existing graph and append new slices")
    parser.add_argument("--full", action="store_true", help="Force full rebuild (ignore existing graph)")
