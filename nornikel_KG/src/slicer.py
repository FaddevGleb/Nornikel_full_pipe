...
Поддерживает incremental режим: добавление новых слайсов без удаления старых.
"""
...
from src.utils.semantic_slicer import build_slicer_llm_config, slice_text_semantic_llm
...
def validate_config_parameters(config):
    ...
    mode = str(slicer_config.get("mode", "llm")).lower()
    if mode not in {"llm", "token"}:
        raise ValueError("slicer.mode must be 'llm' or 'token'")
    
    if mode == "token":
        if "soft_boundary_max_shift" not in slicer_config:
            raise ValueError("Missing required parameter slicer.soft_boundary_max_shift")
...
def create_slug(filename):
    """Создаёт slug из имени файла (транслитерация кириллицы)."""
...
def load_and_validate_file(file_path, allowed_extensions):
    """Загружает и валидирует входной файл."""
...
def get_existing_slice_info(staging_dir):
    """
    Сканирует staging директорию и возвращает:
    - max_order: максимальный order среди существующих слайсов
    - processed_files: множество имён уже обработанных файлов
    - existing_slice_ids: множество ID существующих слайсов
    """
    max_order = 0
    processed_files = set()
    ...
    if not staging_dir.exists():
        return max_order, processed_files, existing_slice_ids
    ...
    for slice_file in staging_dir.glob("*.slice.json"):
        try:
            ...
            if slice_id:
                existing_slice_ids.add(slice_id)
        except Exception:
            pass
...
def clear_staging_directory(staging_dir):
    """Очищает staging директорию от всех слайсов."""
    if not staging_dir.exists():
        return 0
...
def slice_text_with_window(text, max_tokens, soft_boundary, soft_boundary_max_shift,
                           file_name=None, tokenizer_path=None, is_offline=True):
    """Разбивает текст на слайсы с учётом токенов."""
    ...
    while char_pos < len(text):
        ...
        if len(window_tokens) <= max_tokens:
            ...
            logging.info(
                f"Creating slice_{slice_num:03d} "
                f"(tokens {global_token_offset}-{global_token_offset + slice_token_count}) [{progress_pct:.0f}%]"
            )
...
def process_file(file_path, config, global_slice_counter, llm_client=None):
    ...
    try:
        ...
        if mode == "llm":
            if llm_client is None:
                llm_client = LLMClientFactory.create_client(build_slicer_llm_config(config))
            slices_data = slice_text_semantic_llm(
                content,
                slicer_config["max_tokens"],
                llm_client,
                tokenizer_path=tokenizer_path,
                llm_window_chars=slicer_config.get("llm_window_chars", 30000),
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        help="Keep existing slices in staging, add new files with next index",
    )
    ...
    try:
        ...
        # === INCREMENTAL: определяем состояние ===
        max_order, processed_files, existing_slice_ids = get_existing_slice_info(staging_dir)

        if args.incremental:
            # НЕ очищаем staging
            logging.info(f"Incremental mode: keeping existing slices in staging")
            if max_order > 0:
                logging.info(f"  Existing slices: {len(existing_slice_ids)}, max order: {max_order}")
                logging.info(f"  Already processed files: {sorted(processed_files)}")
            global_slice_counter = max_order + 1
        else:
            # Полный rebuild: очищаем staging
        ...
        # === INCREMENTAL: фильтруем уже обработанные файлы ===
        if args.incremental and processed_files:
            new_files = [f for f in input_files if f.name not in processed_files]
            skipped = len(input_files) - len(new_files)
            if skipped > 0:
                logging.info(f"Skipping {skipped} already processed file(s)")
            input_files = new_files

        if not input_files:
            logging.info("No new files to process - all files already sliced")
            return EXIT_SUCCESS

        logging.info(f"Found {len(input_files)} file(s) for processing")

        # Обработка файлов
        total_slices = 0
        slicer_mode = str(config["slicer"].get("mode", "llm")).lower()
        llm_client = None
        if slicer_mode == "llm":
            llm_client = LLMClientFactory.create_client(build_slicer_llm_config(config))
            logging.info("Slicer mode: semantic LLM (Yandex GPT)")

        for file_path in input_files:
            try:
                slices, global_slice_counter = process_file(
                    file_path, config, global_slice_counter, llm_client=llm_client
                )
        ...
        logging.info(f"Processing completed: {total_slices} new slices saved in {staging_dir}")
