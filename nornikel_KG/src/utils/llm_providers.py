...
class LLMClientFactory:
    ...
    def create_client(config):
        ...
        - yandex, openrouter, ollama, vllm, local (через HTTP)
        - local_transformers (напрямую через PyTorch)
        """
        provider = config.get("provider", "yandex").lower()
        valid_providers = ["yandex", "openrouter", "local_transformers", "ollama", "vllm", "local"]
        ...
        if provider == "local_transformers":
            ...
            if provider == "yandex" and not config.get("folder_id"):
                raise ValueError("folder_id is required when provider='yandex'")
