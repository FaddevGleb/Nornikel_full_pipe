...
Supports Yandex GPT, OpenRouter API keys and local models.
"""
import logging
import os
import sys
...
def _inject_env_api_keys(config):
    ...
    Priority:
    1. Environment variable (YANDEX_API_KEY / OPENROUTER_API_KEY)
    ...
    llm_sections = ["slicer", "itext2kg_concepts", "itext2kg_graph", "refiner"]
    for section_name in llm_sections:
        if section_name not in config:
            continue
        section = config[section_name]
        provider = section.get("provider", "yandex").lower()
        if provider == "yandex":
            if yandex_api_key:
                section["api_key"] = yandex_api_key
            if yandex_folder_id:
                section["folder_id"] = yandex_folder_id
            continue
        current_key = section.get("api_key", "")
        if env_api_key and (not current_key or current_key.startswith("sk-...") or current_key.startswith("sk-or-...")):
            section["api_key"] = env_api_key
...
def _validate_slicer_section(section):
    ...
    mode = section.get("mode", "llm").lower()
    if mode not in {"llm", "token"}:
        raise ConfigValidationError("slicer.mode must be 'llm' or 'token'")
    ...
    if mode == "token":
        ...
        _validate_required_fields(section, token_fields, "slicer")
        ...
        if not tokenizer_path or not isinstance(tokenizer_path, str):
            raise ConfigValidationError(
                "slicer.tokenizer_path must be a non-empty string when mode='token'"
            )
    else:
        if section.get("llm_window_chars", 30000) <= 0:
            raise ConfigValidationError("slicer.llm_window_chars must be positive when mode='llm'")
...
def _validate_llm_section(section, section_name):
    ...
    if provider == "yandex":
        if not section.get("folder_id"):
            raise ConfigValidationError(f"{section_name}.folder_id is required for provider='yandex'")
        if not api_key.strip() and not os.getenv("YANDEX_API_KEY"):
            raise ConfigValidationError(
                f"{section_name}.api_key not configured for Yandex. "
                "Set YANDEX_API_KEY or provide api_key in config.toml"
            )
    elif not api_key.strip() or api_key.startswith("sk-...") or api_key.startswith("sk-or-"):
