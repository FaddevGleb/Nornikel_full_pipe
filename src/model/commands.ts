...
import { promptChoice, promptConfirm, promptSelect, promptText, type PromptSelectOption } from "../setup/prompts.js";
...
const API_KEY_PROVIDERS: ApiKeyProviderInfo[] = [
	...
	{ id: "yandex-ai-studio", label: "Yandex AI Studio (YandexGPT)", envVar: "YANDEX_API_KEY" },
...
function apiKeyProviderHint(provider: ApiKeyProviderInfo): string {
	...
	if (provider.id === "yandex-ai-studio") {
...
async function selectApiKeyProvider(): Promise<ApiKeyProviderInfo | undefined> {
	...
	const defaultProvider = API_KEY_PROVIDERS.find((provider) => provider.id === "yandex-ai-studio") ?? API_KEY_PROVIDERS[0];
...
type CustomProviderSetup = {
	 ...
	 */
	authHeader: boolean;
	/** Extra static/resolvable headers to send with every request (e.g. Yandex's `x-folder-id`). */
...
export function buildYandexModelUri(folderId: string, modelName: string, version: string): string {
	return `gpt://${folderId}/${modelName}/${version}`;
}
...
async function promptYandexProviderSetup(): Promise<CustomProviderSetup | undefined> {
	printSection("Yandex AI Studio");
	printInfo("Get an API key and folder ID from https://aistudio.yandex.ru/ (Yandex Cloud console -> AI Studio project).");
	printInfo("API key value supports:");
	printInfo("  - literal secret (stored in models.json)");
	printInfo("  - env var name (resolved at runtime)");
	printInfo("  - !command (executes and uses stdout)");

	const apiKeyConfigRaw = (await promptText("API key / resolver", "YANDEX_API_KEY")).trim();
	const apiKeyConfig = apiKeyConfigRaw || "YANDEX_API_KEY";

	const folderId = (await promptText("Yandex Cloud folder ID (e.g. b1g...)", "")).trim();
	if (!folderId) {
		printWarning("Folder ID is required (Yandex model URIs are gpt://<folder-id>/<model>/<version>).");
		return undefined;
	}

	const baseUrlRaw = await promptText("Base URL", "https://llm.api.cloud.yandex.net/v1");
	const { baseUrl } = normalizeCustomProviderBaseUrl("openai-completions", baseUrlRaw);
	if (!baseUrl) {
		printWarning("Base URL is required.");
		return undefined;
	...
	printInfo("Common model names: yandexgpt, yandexgpt-lite, llama, qwen3-235b-a22b-fp8, gpt-oss-120b");
	const modelNamesRaw = await promptText("Model name(s) (comma-separated, without the gpt://.../ prefix)", "yandexgpt");
	const modelNames = normalizeModelIds(modelNamesRaw);
	if (modelNames.length === 0) {
		printWarning("At least one model name is required.");
		return undefined;
	}

	const version = (await promptText("Model version tag", "latest")).trim() || "latest";
	const modelIds = modelNames.map((modelName) => buildYandexModelUri(folderId, modelName, version));

	const disableLogging = await promptConfirm(
		"Disable Yandex request/response logging (adds x-data-logging-enabled: false)?",
		true,
	);

	const headers: Record<string, string> = { "x-folder-id": folderId };
	if (disableLogging) {
		headers["x-data-logging-enabled"] = "false";
	}

	return {
		providerId: "yandex-ai-studio",
		modelIds,
		baseUrl,
		api: "openai-completions",
		apiKeyConfig,
...
async function verifyCustomProvider(setup: CustomProviderSetup, authPath: string): Promise<void> {
	...
	if (setup.api === "openai-completions" || setup.api === "openai-responses") {
		...
		for (const [key, value] of Object.entries(setup.headers ?? {})) {
			const resolved = await resolveApiKeyConfig(value);
			if (resolved) {
				extraHeaders[key] = resolved;
			}
		}
		try {
			const response = await fetch(url, {
				method: "GET",
				headers: {
					...(setup.authHeader ? { Authorization: `Bearer ${apiKey}` } : {}),
					...extraHeaders,
				},
				signal: controller.signal,
			});
...
async function configureApiKeyProvider(authPath: string, providerId?: string): Promise<boolean> {
	...
	if (provider.id === "yandex-ai-studio") {
		const setup = await promptYandexProviderSetup();
		if (!setup) {
			printInfo("Yandex AI Studio setup cancelled.");
			return false;
		}
		...
		printSuccess("Saved Yandex AI Studio provider.");
		await verifyCustomProvider(setup, authPath);
		return true;
	}
