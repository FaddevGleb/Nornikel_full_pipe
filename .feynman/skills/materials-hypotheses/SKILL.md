...
description: Generate, rank, expand, and regenerate mineral-extraction hypotheses using the local ACCELMAT pipeline (Hypothesis-Generation-for-Materials-Discovery-and-Design-Using-Goal-Driven-and-Constraint-Guided-LLM). Use when the user asks to generate hypotheses about useful-mineral extraction, rank hypotheses, discuss/expand a specific hypothesis, or rerun hypothesis generation with a new goal, constraints, or knowledge graph.
---

# Mineral Extraction Hypotheses (ACCELMAT)
...
Hypotheses focus on **extraction and beneficiation of useful minerals** (добыча и переработка полезных ископаемых), not on designing new alloys or structural materials.

## One-time setup (check before first run)
1. `pip install -r requirements.txt` (if not already installed).
2. Confirm `.env` exists there with `ROUTERAI_API_KEY` or Yandex credentials (see the pipeline's own `README.md`).
...
1. Write `inputs/pipeline_request_<slug>.json`:
   ```json
   {
     ...
     "goal": "<goal: e.g. increase Ni recovery from sulfide ore while limiting tailings volume>",
...
## JSON field meanings
- **Materials** — target useful mineral, ore body, or mineral assemblage
- **Methods_to_develop_the_materials_suggested** — extraction/beneficiation technology (flotation, leaching, smelting, etc.)
- **Reasoning** — justification (grade, recovery, reagents, cost, environment)
...
- Cross-reference `kg_context` entries for ores/minerals mentioned in that hypothesis.
- If the user wants external validation, use Feynman's own research tools (`alpha_search`, `web_search`) to check literature support for the proposed extraction route — this is a value-add on top of the raw pipeline output, not part of ACCELMAT itself.
